    ...
    <section id="services" className="section-container bg-black">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {services.map((service, index) => (
          ...
          >
            {/* Card */}
            <div className="relative h-80 md:h-96 overflow-hidden rounded-lg border border-border">
              {/* Image */}
              <Image
                src={service.image}
                alt={service.title}
              ...
              >
                ...
                <motion.button
                  whileHover={{ x: 5 }}
                  className="w-fit text-accent font-semibold text-sm flex items-center gap-2"
                >
                  Learn More →
                </motion.button>
              </motion.div>
